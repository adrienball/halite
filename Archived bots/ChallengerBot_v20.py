...
def get_best_individual_move(square, available_directions=CARDINAL_DIRECTIONS):
    ...
    squares
    """
    if len(available_directions) == 0 or square.strength <= 5.0 * square.production:
    ...
    if target_square.owner == myID \
            and square.strength <= 5.0 * square.production:
        return Move(square, STILL)
...
def get_distance_factor(square):
    ...
    factor = 5 * math.exp(-distance_to_border / 10)
    return factor
...
def square_opportunity(square):
    threshold = min(max(game_map.width - 10, 0), 25)
    if TURN_COUNT <= threshold:
        return territory_opportunity(square)
    else:
        return production_opportunity(square)


def production_opportunity(square):
...
def territory_opportunity(square):
    normalized_strength = square.strength / MAX_STRENGTH
    normalized_production = square.production / MAX_PRODUCTION
    opportunity = (normalized_production ** 0.25) \
        * ((1 - normalized_strength) ** 3)
    opportunity_factor = get_opportunity_factor(square.owner)
    return opportunity_factor * opportunity
