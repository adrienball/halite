...
def get_best_individual_move(square, available_directions={NORTH, EAST, SOUTH, WEST}):
    # filter out moves that are forbidden
    possible_directions = [d for d in available_directions if is_direction_possible(square, d)]

    if len(possible_directions) == 0 or square.strength <= 5 * square.production:
        return Move(square, STILL)

    if len(possible_directions) == 1:
        return Move(square, possible_directions[0])

    best_opportunity_direction = max(possible_directions,
